...
/**
 * Purpose: Utility class that maintains column information about the test columns in our sharded database table. Notes: Aim is to centralize the
 * column information to make it easier to perform exhaustive testing on our value getters and conversions, etc.
 */
public class MultiShardTestCaseColumn {
    ...
    /**
     * Private, and only, c-tor for MultiShardTestCaseColumn objects. It is private only so that we can tightly control the columns that appear in our
     * test code.
     *
     * @param engineType
     *            String representing the SQL Server Engine data type name.
     * @param columnTypeDeclaration
     ...
     */
    private MultiShardTestCaseColumn(String engineType,
            String columnTypeDeclaration,
            int dbType,
            int fieldLength,
     ...
     *
     * @param sqlTypeName
     *            The SQL Server database engine data type sqlTypeName.
     * @param sqlFieldDeclarationText
     *            The text to use to create this column in SQL Server.
     * @param length
     *            The length of the column (usefulf for char/binary/etc.
     * @param listToAddTo
     *            The list to add the newly created column into.
     */
    private static void addColumnToList(String sqlTypeName,
            String sqlFieldDeclarationText,
            int dbType,
            int length,
            List<MultiShardTestCaseColumn> listToAddTo) {
        String colName = generateTestColumnName(sqlTypeName);
        MultiShardTestCaseColumn toAdd = new MultiShardTestCaseColumn(sqlTypeName, sqlFieldDeclarationText, dbType, length, colName);
     ...
     *
     * @param sqlTypeName
     *            The sql server type sqlTypeName of the column (e.g., numeric).
    ...
    /**
     * Getter that exposes the string to use to declare the column data type when creating the test table. Useful shortcut for allowing us to specify
     * type length parameters directly without pawing through data structures to generate them on the fly.
     */
    ...
    /**
     * Getter that exposes the Sql Type for the column type when accessing it. Useful shortcut for allowing us to pull type information without pawing
    ...
    /**
     * Getter that exposes the field length (if applicable) for the data type when accessing it. Useful shortcut for allowing us to pull variable
     * length data without pawing through column type data structures.
     */
