...
public class ShardMapManagerUpgradeTests {
    ...
    public final void shardMapManagerUpgradeTestInitialize() throws SQLException {
        ...
        try {
            ...
            for (int i = 0; i < ShardMapManagerUpgradeTests.shardDBs.length; i++) {
                try (Statement stmt = conn.createStatement()) {
                    String query = String.format(Globals.DROP_DATABASE_QUERY, ShardMapManagerUpgradeTests.shardDBs[i]);
                    stmt.executeUpdate(query);
                }

                try (Statement stmt = conn.createStatement()) {
                    String query = String.format(Globals.CREATE_DATABASE_QUERY, ShardMapManagerUpgradeTests.shardDBs[i]);
                    stmt.executeUpdate(query);
                }
            }
    ...
    public void getDistinctLocations() {
        ...
        ShardLocation sl1 = new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapManagerUpgradeTests.shardDBs[0]);
        ShardLocation sl2 = new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapManagerUpgradeTests.shardDBs[1]);
        ShardLocation sl3 = new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapManagerUpgradeTests.shardDBs[2]);
    ...
    public void upgradeLsm() {
        ...
        ListShardMap<Integer> listsm = smm.createListShardMap(ShardMapManagerUpgradeTests.shardMapNames[0], ShardKeyType.Int32);
    ...
    public final void testLockingFixInVersion1_2() {
        ...
        RangeShardMap<Integer> rsm = smm.createRangeShardMap(ShardMapManagerUpgradeTests.shardMapNames[1], ShardKeyType.Int32);
        assert rsm != null;

        Shard s = rsm.createShard(new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapManagerUpgradeTests.shardDBs[0]));
        assert s != null;
        ...
        assert MappingLockToken.opEquality(MappingLockToken.NoLock, rsm.getMappingLockOwner(rsm.getMappingForKey(5)));
        assert MappingLockToken.opEquality(MappingLockToken.NoLock, rsm.getMappingLockOwner(rsm.getMappingForKey(15)));
    ...
    private void verifyGlobalStore(ShardMapManager smm,
        ...
        if (targetVersion != null && Version.isFirstGreaterThan(new Version(1, 1), targetVersion)) {
            ShardManagementException sme = AssertExtensions.assertThrows(() -> smm.createListShardMap(shardMapName, ShardKeyType.Int32));
