...
/**
 * Attaches the given shard map and shard information to the GSM database and update shard location in LSM. UndoLocalSourceExecute is not implemented
 * for this operation because UpdateShardLocal operation is needed irrespective of the success/error condition, it updates Shards table with correct
...
public class AttachShardOperation extends StoreOperation {
     ...
     * @param shardMap
     *            Shard map for which to attach shard.
     * @param shard
     *            Shard to attach.
     */
    public AttachShardOperation(ShardMapManager shardMapManager,
    ...
    public StoreConnectionInfo getStoreConnectionInfo() {
        ...
        tempVar.setSourceLocation(
                this.getUndoStartState().getValue() <= StoreOperationState.UndoLocalSourceBeginTransaction.getValue() ? shard.getLocation() : null);
        return tempVar;
    }
    ...
    public void handleDoGlobalPreLocalExecuteError(StoreResults result) {
        ...
        throw StoreOperationErrorHandler.onShardMapErrorGlobal(result, shardMap, shard, ShardManagementErrorCategory.Recovery,
    ...
    public StoreResults doLocalSourceExecute(IStoreTransactionScope ts) {
        ...
        ts.executeCommandBatch(
                SqlUtils.filterUpgradeCommands(SqlUtils.getUpgradeLocalScript(), GlobalConstants.LsmVersionClient, checkResult.getStoreVersion()));
    ...
    public void handleDoLocalSourceExecuteError(StoreResults result) {
        ...
        switch (result.getResult()) {
            ...
            case ShardDoesNotExist:
                throw StoreOperationErrorHandler.onShardMapErrorLocal(result, shardMap, shard.getLocation(), ShardManagementErrorCategory.ShardMap,
                        StoreOperationErrorHandler.operationNameFromStoreOperationCode(this.getOperationCode()),
                        StoreOperationRequestBuilder.SP_UPDATE_SHARD_LOCAL);

            default:
                throw new ShardManagementException(ShardManagementErrorCategory.ShardMapManager, ShardManagementErrorCode.StorageOperationFailure,
                        Errors._Store_SqlExceptionLocal, getOperationName());
        }
    }
    ...
    public StoreResults undoGlobalPostLocalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARDS_GLOBAL_END,
                StoreOperationRequestBuilder.addShardGlobal(this.getId(), this.getOperationCode(), true, shardMap, shard)); // undo
    }
