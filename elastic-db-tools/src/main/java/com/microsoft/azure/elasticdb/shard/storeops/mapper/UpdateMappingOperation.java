...
public class UpdateMappingOperation extends StoreOperation {
     ...
     *            Id of lock owner.
     */
    public UpdateMappingOperation(ShardMapManager shardMapManager,
            ...
            UUID lockOwnerId) {
        this(shardMapManager, UUID.randomUUID(), StoreOperationState.UndoBegin, operationCode, shardMap, mappingSource, mappingTarget, patternForKill,
                lockOwnerId, null, null);
    }
    ...
    public UpdateMappingOperation(ShardMapManager shardMapManager,
        ...
        errorCategory = (operationCode == StoreOperationCode.UpdateRangeMapping || operationCode == StoreOperationCode.UpdateRangeMappingWithOffline)
                ? ShardManagementErrorCategory.RangeShardMap
                : ShardManagementErrorCategory.ListShardMap;

        updateLocation = this.mappingSource.getStoreShard().getId() != this.mappingTarget.getStoreShard().getId();
    ...
    public StoreConnectionInfo getStoreConnectionInfo() {
        ...
        tempVar.setSourceLocation(this.getUndoStartState().getValue() <= StoreOperationState.UndoLocalSourceBeginTransaction.getValue()
                ...
                : null);
        tempVar.setTargetLocation(
                (updateLocation && this.getUndoStartState().getValue() <= StoreOperationState.UndoLocalTargetBeginTransaction.getValue())
    ...
    public StoreResults doGlobalPreLocalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_GLOBAL_BEGIN,
                StoreOperationRequestBuilder.updateShardMappingGlobal(this.getId(), this.getOperationCode(), false, patternForKill, shardMap,
                        mappingSource, mappingTarget, lockOwnerId)); // undo
    }
    ...
    public void handleDoGlobalPreLocalExecuteError(StoreResults result) {
        ...
        throw StoreOperationErrorHandler.onShardMapperErrorGlobal(result, shardMap,
                result.getResult() == StoreResult.MappingRangeAlreadyMapped ? mappingTarget.getStoreShard() : mappingSource.getStoreShard(),
                errorCategory, StoreOperationErrorHandler.operationNameFromStoreOperationCode(this.getOperationCode()),
    ...
    public StoreResults doLocalSourceExecute(IStoreTransactionScope ts) {
        ...
        if (updateLocation) {
            result = ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                    StoreOperationRequestBuilder.removeShardMappingLocal(this.getId(), false, shardMap, mappingSource));
        }
        else {
            result = ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                    StoreOperationRequestBuilder.updateShardMappingLocal(this.getId(), false, shardMap, mappingSource, mappingTarget));
        }
    ...
    public void handleDoLocalSourceExecuteError(StoreResults result) {
        ...
        // StoreResult.StoreVersionMismatch
        // StoreResult.MissingParametersForStoredProcedure
        throw StoreOperationErrorHandler.onShardMapperErrorLocal(result, mappingSource.getStoreShard().getLocation(),
    ...
    public StoreResults doLocalTargetExecute(IStoreTransactionScope ts) {
        ...
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                StoreOperationRequestBuilder.addShardMappingLocal(this.getId(), false, shardMap, mappingTarget));
    }
    ...
    public void handleDoLocalTargetExecuteError(StoreResults result) {
        ...
        // StoreResult.StoreVersionMismatch
        // StoreResult.MissingParametersForStoredProcedure
        throw StoreOperationErrorHandler.onShardMapperErrorLocal(result, mappingTarget.getStoreShard().getLocation(),
                StoreOperationErrorHandler.operationNameFromStoreOperationCode(this.getOperationCode()),
                StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL);
    }
    ...
    public void handleDoGlobalPostLocalExecuteError(StoreResults result) {
        ...
        // StoreResult.StoreVersionMismatch
        // StoreResult.MissingParametersForStoredProcedure
        throw StoreOperationErrorHandler.onShardMapperErrorGlobal(result, shardMap, mappingSource.getStoreShard(), errorCategory,
                StoreOperationErrorHandler.operationNameFromStoreOperationCode(this.getOperationCode()),
    ...
    public void doGlobalPostLocalUpdateCache(StoreResults result) {
        ...
        // Add to cache.
        this.getShardMapManager().getCache().addOrUpdateMapping(mappingTarget, CacheStoreMappingUpdatePolicy.OverwriteExisting);
    }
    ...
    public StoreResults undoLocalSourceExecute(IStoreTransactionScope ts) {
        StoreMapping dsmSource = new StoreMapping(mappingSource.getId(), shardMap.getId(), mappingSource.getMinValue(), mappingSource.getMaxValue(),
                mappingSource.getStatus(), lockOwnerId, new StoreShard(mappingSource.getStoreShard().getId(), this.getOriginalShardVersionRemoves(),
                        shardMap.getId(), mappingSource.getStoreShard().getLocation(), mappingSource.getStoreShard().getStatus()));

        if (updateLocation) {
            return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                    StoreOperationRequestBuilder.addShardMappingLocal(this.getId(), true, shardMap, dsmSource));
        }
        else {
            StoreMapping dsmTarget = new StoreMapping(mappingTarget.getId(), shardMap.getId(), mappingTarget.getMinValue(),
                    mappingTarget.getMaxValue(), mappingTarget.getStatus(), lockOwnerId,
                    new StoreShard(mappingTarget.getStoreShard().getId(), this.getOriginalShardVersionRemoves(), shardMap.getId(),
                            mappingTarget.getStoreShard().getLocation(), mappingTarget.getStoreShard().getStatus()));

            return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                    StoreOperationRequestBuilder.updateShardMappingLocal(this.getId(), true, shardMap, dsmTarget, dsmSource));
        }
    }
    ...
    public StoreResults undoLocalTargetExecute(IStoreTransactionScope ts) {
        StoreMapping dsmTarget = new StoreMapping(mappingTarget.getId(), shardMap.getId(), mappingTarget.getMinValue(), mappingTarget.getMaxValue(),
                mappingTarget.getStatus(), lockOwnerId, new StoreShard(mappingTarget.getStoreShard().getId(), this.getOriginalShardVersionAdds(),
                        shardMap.getId(), mappingTarget.getStoreShard().getLocation(), mappingTarget.getStoreShard().getStatus()));

        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_LOCAL,
                StoreOperationRequestBuilder.removeShardMappingLocal(this.getId(), true, shardMap, dsmTarget));
    }
    ...
    public StoreResults undoGlobalPostLocalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_GLOBAL_END,
                StoreOperationRequestBuilder.updateShardMappingGlobal(this.getId(), this.getOperationCode(), true, patternForKill, shardMap,
                        mappingSource, mappingTarget, lockOwnerId)); // undo
    }
    ...
    private void killConnectionsOnSourceShard() {
        SqlUtils.withSqlExceptionHandling(() -> {
            String sourceShardConnectionString = this.getConnectionStringForShardLocation(mappingSource.getStoreShard().getLocation());

            StoreResults result = null;

            try (IStoreConnection connectionForKill = this.getShardMapManager().getStoreConnectionFactory()
                ...
                try (IStoreTransactionScope ts = connectionForKill.getTransactionScope(StoreTransactionScopeKind.NonTransactional)) {
                    result = ts.executeOperation(StoreOperationRequestBuilder.SP_KILL_SESSIONS_FOR_SHARD_MAPPING_LOCAL,
            ...
            if (result.getResult() != StoreResult.Success) {
                ...
                // StoreResult.StoreVersionMismatch
                // StoreResult.MissingParametersForStoredProcedure
                throw StoreOperationErrorHandler.onShardMapErrorLocal(result, shardMap, mappingSource.getStoreShard().getLocation(), errorCategory,
                        StoreOperationErrorHandler.operationNameFromStoreOperationCode(this.getOperationCode()),
