...
public class UpgradeStoreGlobalOperation extends StoreOperationGlobal {
     ...
     */
    public UpgradeStoreGlobalOperation(ShardMapManager shardMapManager,
    ...
    public StoreResults doGlobalExecute(IStoreTransactionScope ts) {
        log.info("ShardMapManagerFactory {} Started upgrading Global Shard Map structures.", this.getOperationName());
        ...
        if (Version.isFirstGreaterThan(targetVersion, checkResult.getStoreVersion())) {
            ...
            ts.executeCommandBatch(SqlUtils.filterUpgradeCommands(SqlUtils.getUpgradeGlobalScript(), targetVersion, checkResult.getStoreVersion()));
            ...
            log.info("ShardMapManagerFactory {} Finished upgrading Global Shard Map. Duration:{}", this.getOperationName(),
                    stopwatch.elapsed(TimeUnit.MILLISECONDS));
        }
        else {
            log.error(
                    "ShardMapManagerFactory {} Global Shard Map is at a version {} equal to or higher than"
                            + "Client library version {}, skipping upgrade.",
                    this.getOperationName(), (checkResult.getStoreVersion() == null) ? "" : checkResult.getStoreVersion().toString(),
