...
public final class SqlShardMapManagerCredentials {
    ...
    /**
     * Instantiates the object that holds the credentials for accessing SQL Servers containing the shard map manager data.
     *
     * @param connectionString
     *            Connection string for ShardId map manager data source.
     */
    public SqlShardMapManagerCredentials(String connectionString) {
        ...
        if (StringUtilsLocal.isNullOrEmpty(connectionStringBuilder.getDataSource())) {
            throw new IllegalArgumentException(
                    StringUtilsLocal.formatInvariant(Errors._SqlShardMapManagerCredentials_ConnectionStringPropertyRequired, "DataSource"),
        ...
        if (StringUtilsLocal.isNullOrEmpty(connectionStringBuilder.getDatabaseName())) {
            throw new IllegalArgumentException(
                    StringUtilsLocal.formatInvariant(Errors._SqlShardMapManagerCredentials_ConnectionStringPropertyRequired, "Initial Catalog"),
                    new Throwable("connectionString"));
        }
        ...
        connectionStringShardMapManager = new SqlConnectionStringBuilder(connectionStringBuilder.getConnectionString());

        connectionStringShardMapManager.setApplicationName(ApplicationNameHelper.addApplicationNameSuffix(
                connectionStringShardMapManager.getApplicationName(), GlobalConstants.ShardMapManagerInternalConnectionSuffixGlobal));

        connectionStringShard = new SqlConnectionStringBuilder(connectionStringBuilder.getConnectionString());

        connectionStringShard.remove("Data Source");
        connectionStringShard.remove("Initial Catalog");

        connectionStringShard.setApplicationName(ApplicationNameHelper.addApplicationNameSuffix(connectionStringShard.getApplicationName(),
    ...
    public static void ensureCredentials(SqlConnectionStringBuilder connectionString,
        ...
        if (connectionString.containsKey(ShardMapUtils.Authentication)
                && connectionString.getItem(ShardMapUtils.Authentication).toString().equals(ShardMapUtils.ActiveDirectoryIntegratedStr)) {
            return;
        }
        ...
        if (StringUtilsLocal.isNullOrEmpty(connectionString.getUser())) {
            throw new IllegalArgumentException(
                    StringUtilsLocal.formatInvariant(Errors._SqlShardMapManagerCredentials_ConnectionStringPropertyRequired, "UserID"),
                    new Throwable(parameterName));
        }

        // Password must be set when integrated authentication is disabled.
        if (!connectionString.getIntegratedSecurity() && StringUtilsLocal.isNullOrEmpty(connectionString.getPassword())) {
            throw new IllegalArgumentException(
                    StringUtilsLocal.formatInvariant(Errors._SqlShardMapManagerCredentials_ConnectionStringPropertyRequired, "Password"),
                    new Throwable(parameterName));
        }
    }
    ...
    public String getShardMapManagerLocation() {
        return StringUtilsLocal.formatInvariant("[DataSource=%1$s Database=%2$s]", connectionStringShardMapManager.getDataSource(),
