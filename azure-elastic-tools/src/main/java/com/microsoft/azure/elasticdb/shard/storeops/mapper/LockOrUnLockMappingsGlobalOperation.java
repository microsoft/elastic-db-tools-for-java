...
public class LockOrUnLockMappingsGlobalOperation extends StoreOperationGlobal {
     ...
     *            Error category.
     */
    public LockOrUnLockMappingsGlobalOperation(ShardMapManager shardMapManager,
    ...
    public StoreResults doGlobalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_LOCK_OR_UN_LOCK_SHARD_MAPPINGS_GLOBAL,
                StoreOperationRequestBuilder.lockOrUnLockShardMappingsGlobal(shardMap, mapping, lockOwnerId, lockOwnerIdOpType));
    }
    ...
    public void handleDoGlobalExecuteError(StoreResults result) {
        ...
        throw StoreOperationErrorHandler.onShardMapperErrorGlobal(result, shardMap, mapping == null ? null : mapping.getStoreShard(), errorCategory,
                this.getOperationName(), StoreOperationRequestBuilder.SP_LOCK_OR_UN_LOCK_SHARD_MAPPINGS_GLOBAL);
    }
    ...
    protected void undoPendingStoreOperations(StoreLogEntry logEntry) throws Exception {
        try (IStoreOperation op = shardMapManager.getStoreOperationFactory().fromLogEntry(shardMapManager, logEntry)) {
            op.undoOperation();
        }
    }
}