...
public class ReplaceMappingsOperation extends StoreOperation {
     ...
     */
    public ReplaceMappingsOperation(ShardMapManager shardMapManager,
    ...
    public StoreResults doGlobalPreLocalExecute(IStoreTransactionScope ts) {
        // undo
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_GLOBAL_BEGIN, StoreOperationRequestBuilder
                .replaceShardMappingsGlobal(this.getId(), this.getOperationCode(), false, shardMap, mappingsSource, mappingsTarget));
    }
    ...
    public void handleDoGlobalPreLocalExecuteError(StoreResults result) {
        ...
        if (result.getResult() == StoreResult.MappingDoesNotExist) {
            for (StoreMapping mappingSource : mappingsSource.stream().map(Pair::getLeft).collect(Collectors.toList())) {
    ...
    public void handleDoLocalSourceExecuteError(StoreResults result) {
        ...
        // StoreResult.StoreVersionMismatch
        // StoreResult.MissingParametersForStoredProcedure
        throw StoreOperationErrorHandler.onShardMapperErrorLocal(result, mappingsSource.get(0).getLeft().getStoreShard().getLocation(),
    ...
    public StoreResults doGlobalPostLocalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_GLOBAL_END, StoreOperationRequestBuilder
                .replaceShardMappingsGlobal(this.getId(), this.getOperationCode(), false, shardMap, mappingsSource, mappingsTarget)); // undo
    }
    ...
    public void doGlobalPostLocalUpdateCache(StoreResults result) {
        ...
        for (Pair<StoreMapping, UUID> ssm : mappingsTarget) {
            this.getShardMapManager().getCache().addOrUpdateMapping(ssm.getLeft(), CacheStoreMappingUpdatePolicy.OverwriteExisting);
        }
    }
    ...
    public StoreResults undoGlobalPostLocalExecute(IStoreTransactionScope ts) {
        return ts.executeOperation(StoreOperationRequestBuilder.SP_BULK_OPERATION_SHARD_MAPPINGS_GLOBAL_END, StoreOperationRequestBuilder
                .replaceShardMappingsGlobal(this.getId(), this.getOperationCode(), true, shardMap, mappingsSource, mappingsTarget)); // undo
    }
