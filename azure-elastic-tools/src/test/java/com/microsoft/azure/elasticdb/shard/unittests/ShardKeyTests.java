...
public class ShardKeyTests {
    ...
    private final ShardKey[] orderedGuidsDescending = {ShardKey.getMaxGuid(), new ShardKey(UUID.fromString("00000000-0000-0000-0000-010000000000")),
     ...
     */
    private HashMap<ShardKeyType, Integer> shardKeyTypeLength = new HashMap<ShardKeyType, Integer>() {
        {
    ...
    /**
     * Verifies that ShardKey.FromRawValue(keyType, rawValue) returns the correct ShardKey and ShardKey.Value if extra zeroes are added to the end of
     * rawValue
     */
    @Test
    public void testShardKeyDeserializationAddTrailingZeroes() {
        for (ShardKeyInfo shardKeyInfo : ShardKeyInfo.allTestShardKeyInfos) {
            ...
            if (shardKeyInfo.getRawValue() != null && shardKeyInfo.getRawValue().length != dataTypeLength) {
                // Add trailing zeroes
    ...
    public void testShardKeyTypeInfo() {
        for (ShardKeyTypeInfo shardKeyTypeInfo : ShardKeyTypeInfo.shardKeyTypeInfos.values()) {
            ...
            if (shardKeyTypeInfo.keyType != ShardKeyType.DateTime && shardKeyTypeInfo.keyType != ShardKeyType.DateTimeOffset) {
                ...
                AssertExtensions.assertSequenceEqual(new byte[0], shardKeyTypeInfo.minShardKey.getRawValue());
            }
    ...
    public final void testGuidOrdering() {
        for (int i = 1; i < orderedGuidsDescending.length - 1; i++) {
            Assert.assertTrue(String.format("Expected %1$s to be great than %2$s", orderedGuidsDescending[i - 1], orderedGuidsDescending[i]),
    ...
    private <KeyT> void testShardKeyGeneric(ShardKeyType keyType,
            KeyT inputValue,
            Class realType) {
        // Exercise DetectType
