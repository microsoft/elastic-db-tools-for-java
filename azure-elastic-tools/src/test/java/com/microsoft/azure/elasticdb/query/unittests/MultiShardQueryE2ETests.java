...
/**
 * Tests for end to end scenarios where a user connects to his shards, executes commands against them and receives results Purpose: Basic End-To-End
 * test scenarios for the cross shard query client library. Notes: Tests currently assume there's a running sqlservr instance. Everything will be
 * automated once we integrate with the larger framework. Currently the tests use the same methods to create shards as MultiShardResultSetTests.
 */
public class MultiShardQueryE2ETests {
            ...
            try (MultiShardResultSet sdr = stmt.executeQuery()) {
                ...
                while (sdr.next()) {
                    ...
                    String logRecord = String.format(
                            "RecordRetrieved: dbNameField: %1$s, TestIntField:"
                                    + " %2$s, TestBigIntField: %3$s, shardIdPseudoColumnField: %4$s, RecordCount:" + " %5$s",
    ...
    private void testSelectNoRows(String commandText,
    ...
    private void testSelectNonQuery(String commandText,
            MultiShardExecutionPolicy policy) {
        try (MultiShardStatement stmt = shardConnection.createCommand()) {
            stmt.setCommandText(commandText);
            stmt.setExecutionPolicy(policy);
    ...
    public final void testSelect_Failure_PartialResults() {
        MultiShardAggregateException e = testSelectFailure("raiserror('blah', 16, 0)", MultiShardExecutionPolicy.PartialResults);
    ...
    public final void testSelect_Failure_CompleteResults() {
        MultiShardAggregateException e = testSelectFailure("raiserror('blah', 16, 0)", MultiShardExecutionPolicy.CompleteResults);
    ...
    private String getPartialFailureQuery() {
        List<ShardLocation> shardLocations = shardMap.getShards().stream().map(Shard::getLocation).collect(Collectors.toList());
        ...
        assert 1 == shardLocations.stream().filter(l -> l.getDatabase().equals(chosenShardLocation.getDatabase())).count();
        ...
        return String.format("if db_name() = '%1$s' raiserror('blah', 16, 0) else select 1", shardLocations.get(0).getDatabase());
    }

    /**
     * Basic test for async api(s), Also demonstrates the async pattern of this library. The Sync api is implicitly tested in
     * MultiShardResultSetTests::TestSimpleSelect.
     */
    @Test
    @Category(value = ExcludeFromGatedCheckin.class)
        ...
        try (MultiShardStatement stmt = shardConnection.createCommand()) {
            ...
            try (MultiShardResultSet sdr = execAsync(stmt).call()) {
                ...
                while (sdr.next()) {
                    ...
                    int testIntField = sdr.getInt(2);
                    long testBigIntField = sdr.getLong(3);
                    log.info("RecordRetrieved: dbNameField: {}, TestIntField: {}, TestBigIntField: {}, Count:" + " {}", dbNameField, testIntField,
                            testBigIntField, recordsRetrieved);
                }
    ...
    public final void testShardNamePseudoColumnOption() {
        ...
        for (boolean pseudoColumnPresent : pseudoColumnOptions) {
            try (MultiShardConnection conn = new MultiShardConnection(MultiShardTestUtils.MULTI_SHARD_CONN_STRING,
                ...
                try (MultiShardStatement stmt = conn.createCommand()) {
                    ...
                    stmt.setExecutionOptions(
                            pseudoColumnPresent ? MultiShardExecutionOptions.IncludeShardNameColumn : MultiShardExecutionOptions.None);
        ...
        try {
            ...
            try (MultiShardStatement stmt = shardConnection.createCommand()) {
                ...
                try (MultiShardResultSet sdr = execAsync(stmt).call()) {
                    ...
                    Assert.assertTrue(String.format("Expected 1 or 2 execution errors, but saw %1$s", sdr.getMultiShardExceptions().size()),
                            sdr.getMultiShardExceptions().size() == 1 || sdr.getMultiShardExceptions().size() == 2);
    ...
    public final void testQueryShardsTvpParam() throws Exception {
        try {
            // Install schema
            String createTbl = "CREATE TABLE dbo.PageView (PageViewID BIGINT NOT NULL, PageViewCount BIGINT NOT NULL);" + "\r\n"
            ...
            String dropSchema = "IF EXISTS" + " (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[procMergePageView]') AND"
    ...
    /**
     * Validate the MultiShardConnectionString's connectionString param. - Shouldn't be null. - No DataSource/InitialCatalog should be set. -
    ...
    public final void testInvalidMultiShardConnectionString() throws SQLException {
        ...
        int length = ApplicationNameHelper.MAX_APPLICATION_NAME_LENGTH - MultiShardConnection.ApplicationNameSuffix.length();
        for (int i = 0; i < length; i++) {
            applicationStringBldr.append('x');
        }
        String applicationName = applicationStringBldr.toString();
        SqlConnectionStringBuilder connStringBldr = new SqlConnectionStringBuilder(MultiShardTestUtils.MULTI_SHARD_CONN_STRING);
        connStringBldr.setApplicationName(applicationName);
        MultiShardConnection conn = new MultiShardConnection(connStringBldr.getConnectionString(), shardArray);

        String updatedApplicationName = (new SqlConnectionStringBuilder(conn.getShardConnections().get(0).getRight().getMetaData().getURL()))
                .getApplicationName();
        Assert.assertTrue(String.format("ApplicationName not appended with %1$s!", MultiShardConnection.ApplicationNameSuffix),
                updatedApplicationName.length() == ApplicationNameHelper.MAX_APPLICATION_NAME_LENGTH
