...
public class StoreOperationErrorHandler {
    ...
    switch (result.getResult()) {
      case ShardMapExists:
        ...
        return new ShardManagementException(ShardManagementErrorCategory.ShardMapManager,
            ...
            Errors._Store_ShardMap_AlreadyExistsGlobal, shardMap.getName(), storedProcName,
            operationName);

      case ShardMapHasShards:
        assert shardMap != null;
        return new ShardManagementException(ShardManagementErrorCategory.ShardMapManager,
            ShardManagementErrorCode.ShardMapHasShards, Errors._Store_ShardMap_ContainsShardsGlobal,
            shardMap.getName(), storedProcName, operationName);
  ...
  public static ShardManagementException OnShardMapErrorGlobal(StoreResults result,
    ...
    switch (result.getResult()) {
      case ShardMapDoesNotExist:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardMapDoesNotExist,
            Errors._Store_ShardMap_DoesNotExistGlobal, shardMap.getName(), storedProcName,
            operationName, shard != null ? shard.getLocation().toString() : "*");

      case ShardExists:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardAlreadyExists, Errors._Store_Shard_AlreadyExistsGlobal,
      ...
      case ShardLocationExists:
        return new ShardManagementException(errorCategory,
            ...
            Errors._Store_Shard_LocationAlreadyExistsGlobal, shard.getLocation(),
            shardMap.getName(), storedProcName, operationName);

      case ShardDoesNotExist:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardDoesNotExist, Errors._Store_Shard_DoesNotExistGlobal,
            shard.getLocation(), shardMap.getName(), storedProcName, operationName);

      case ShardVersionMismatch:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardVersionMismatch,
            Errors._Store_Shard_VersionMismatchGlobal, shard.getLocation(), shardMap.getName(),
            storedProcName, operationName);

      case ShardHasMappings:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardHasMappings, Errors._Store_Shard_HasMappingsGlobal,
            shard.getLocation(), shardMap.getName(), storedProcName, operationName);
    ...
    switch (result.getResult()) {
      case UnableToKillSessions:
        return new ShardManagementException(errorCategory,
            ...
            Errors._Store_ShardMapper_UnableToKillSessions, location, shardMap.getName(),
            operationName, storedProcName, location);
      ...
      case ShardDoesNotExist:
        ...
        return new ShardManagementException(ShardManagementErrorCategory.Recovery,
            ShardManagementErrorCode.ShardDoesNotExist, Errors._Store_Validate_ShardDoesNotExist,
    ...
    switch (result.getResult()) {
      ...
      case MappingDoesNotExist:
        return new ShardManagementException(errorCategory,
            ...
            Errors._Store_ShardMapper_MappingDoesNotExistGlobal, shard.getLocation(),
            shardMap.getName(), storedProcName, operationName);

      case MappingRangeAlreadyMapped:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.MappingRangeAlreadyMapped,
            Errors._Store_ShardMapper_MappingPointOrRangeAlreadyMapped, shard.getLocation(),
            shardMap.getName(), "Range", storedProcName, operationName);

      case MappingPointAlreadyMapped:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.MappingPointAlreadyMapped,
            Errors._Store_ShardMapper_MappingPointOrRangeAlreadyMapped, shard.getLocation(),
            shardMap.getName(), "Point", storedProcName, operationName);
      ...
      case MappingIsAlreadyLocked:
        return new ShardManagementException(errorCategory,
            ...
            Errors._Store_ShardMapper_LockMappingAlreadyLocked, shard.getLocation(),
            shardMap.getName(), storedProcName, operationName);

      case MappingLockOwnerIdDoesNotMatch:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.MappingLockOwnerIdDoesNotMatch,
            Errors._Store_ShardMapper_LockOwnerDoesNotMatch, shard.getLocation(),
            shardMap.getName(), storedProcName, operationName);

      case MappingIsNotOffline:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.MappingIsNotOffline,
            Errors._Store_ShardMapper_MappingIsNotOffline, shard.getLocation(), shardMap.getName(),
            storedProcName, operationName);
    ...
    switch (result.getResult()) {
      case ShardMapDoesNotExist:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ShardManagementErrorCode.ShardMapDoesNotExist,
            Errors._Store_Validate_ShardMapDoesNotExist, shardMap.getName(), location,
      ...
      case ShardVersionMismatch:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ShardManagementErrorCode.ShardVersionMismatch,
            Errors._Store_Validate_ShardVersionMismatch, location, shardMap.getName(),
            operationName, storedProcName);

      case MappingDoesNotExist:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ShardManagementErrorCode.MappingDoesNotExist,
            Errors._Store_Validate_MappingDoesNotExist, location, shardMap.getName(), operationName,
            storedProcName);

      case MappingIsOffline:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ShardManagementErrorCode.MappingIsOffline, Errors._Store_Validate_MappingIsOffline,
            location, shardMap.getName(), operationName, storedProcName);
  ...
  public static ShardManagementException OnShardSchemaInfoErrorGlobal(StoreResults result,
    ...
    switch (result.getResult()) {
      case SchemaInfoNameConflict:
        throw new SchemaInfoException(SchemaInfoErrorCode.SchemaInfoNameConflict,
            Errors._Store_SchemaInfo_NameConflict, shardMapName);
  ...
  public static ShardManagementException OnRecoveryErrorLocal(StoreResults result,
    ...
    switch (result.getResult()) {
      case ShardMapDoesNotExist:
        return new ShardManagementException(errorCategory,
            ShardManagementErrorCode.ShardMapDoesNotExist, Errors._Store_ShardMap_DoesNotExistLocal,
            shardMap.getName(), location, storedProcName, operationName);
    ...
    switch (result.getResult()) {
      ...
      case MissingParametersForStoredProcedure:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ...
            Errors._Store_MissingSprocParametersGlobal, operationName, storedProcName);

      default:
        //Debug.Fail("Unexpected error code found.");
        return new ShardManagementException(ShardManagementErrorCategory.General,
            ShardManagementErrorCode.UnexpectedError, Errors._Store_UnexpectedErrorGlobal);
    }
  }
    ...
    switch (result.getResult()) {
      case StoreVersionMismatch:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ...
            (result.getStoreVersion() != null) ? result.getStoreVersion().toString() : "", location,
            GlobalConstants.LsmVersionClient, (result.getStoreVersion() != null) ? (
            Version.isFirstGreaterThan(result.getStoreVersion(), GlobalConstants.LsmVersionClient)
                ? "library" : "store") : "store");

      case MissingParametersForStoredProcedure:
        return new ShardManagementException(ShardManagementErrorCategory.Validation,
            ShardManagementErrorCode.LocalStoreOperationInsufficientParameters,
            Errors._Store_MissingSprocParametersLocal, operationName, location, storedProcName);

      default:
        //Debug.Fail("Unexpected error code found.");
        return new ShardManagementException(ShardManagementErrorCategory.General,
            ShardManagementErrorCode.UnexpectedError, Errors._Store_UnexpectedErrorLocal, location);
    }
  }
