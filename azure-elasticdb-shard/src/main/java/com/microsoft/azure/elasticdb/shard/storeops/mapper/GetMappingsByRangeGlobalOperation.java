...
public class GetMappingsByRangeGlobalOperation extends StoreOperationGlobal {
     ...
     */
    public GetMappingsByRangeGlobalOperation(ShardMapManager shardMapManager, String operationName, IStoreShardMap shardMap, IStoreShard shard, ShardRange range, ShardManagementErrorCategory errorCategory, boolean cacheResults, boolean ignoreFailure) {
    ...
    public void HandleDoGlobalExecuteError(IStoreResults result) {
        ...
        if (!_ignoreFailure || result.getResult() != StoreResult.ShardMapDoesNotExist) {
            ...
            throw StoreOperationErrorHandler.OnShardMapperErrorGlobal(result, _shardMap, _shard, _errorCategory, this.getOperationName(), StoreOperationRequestBuilder.SpGetAllShardMappingsGlobal);
        }
    }
