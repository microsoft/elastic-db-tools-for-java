...
public class SchemaInfoCollectionTests {
  ...
  public void testAddAndLookupAndDeleteSchemaInfo() {
    ...
    SchemaInfoException siex = AssertExtensions.<SchemaInfoException>assertThrows(
    ...
    siex = AssertExtensions.<SchemaInfoException>assertThrows(
    ...
    IllegalArgumentException arex = AssertExtensions
        .<IllegalArgumentException>assertThrows(() -> si.add((ShardedTableInfo) null));

    arex = AssertExtensions
        .<IllegalArgumentException>assertThrows(() -> si.add((ReferenceTableInfo) null));
    ...
    // exception.
    siex = AssertExtensions.<SchemaInfoException>assertThrows(() -> siCollection.add(mdName, si));
    ...
    siex =
        AssertExtensions.<SchemaInfoException>assertThrows(() -> siCollection.get(mdName + "Fail"));
    ...
    siex = AssertExtensions.<SchemaInfoException>assertThrows(() -> siCollection.get(mdName));
    assert SchemaInfoErrorCode.SchemaInfoNameDoesNotExist == siex.getErrorCode();
  ...
  public void testSetSchemaInfoWithSpecialChars() {
    ...
    ShardedTableInfo sti =
        new ShardedTableInfo(NewNameWithSpecialChars(), NewNameWithSpecialChars());
  ...
  public void testUpdateSchemaInfo() {
    ...
    SchemaInfoException siex =
        AssertExtensions.<SchemaInfoException>assertThrows(() -> siCollection.replace(mdName, si));
    assert SchemaInfoErrorCode.SchemaInfoNameDoesNotExist == siex.getErrorCode();
  ...
  public void serializeCompatibility() {
    ...
    assert expectedSerializedSchemaInfo == actualSerializedSchemaInfo;
  ...
  public void deserializeCompatibilityV100() {
    ...
    assert "r1" == schemaInfo.getReferenceTables().stream().findFirst().get().getSchemaName();
    assert "r2" == schemaInfo.getReferenceTables().stream().findFirst().get().getTableName();
    assert 1 == schemaInfo.getShardedTables().size();
    assert "s1" == schemaInfo.getShardedTables().stream().findFirst().get().getSchemaName();
    assert "s2" == schemaInfo.getShardedTables().stream().findFirst().get().getTableName();
    assert "s3" == schemaInfo.getShardedTables().stream().findFirst().get().getKeyColumnName();
    ...
    assert expectedFinalSchemaInfo == actualFinalSchemaInfo;
  }
