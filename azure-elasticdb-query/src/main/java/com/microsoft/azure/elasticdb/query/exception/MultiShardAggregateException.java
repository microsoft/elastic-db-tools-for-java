...
public class MultiShardAggregateException extends RuntimeException implements Serializable {
    ...
    public String toString() {
        ...
        for (int i = 0; i < _innerExceptions.size(); i++) {
            text = String.format(Locale.getDefault(), "%1$s%2$s---> (Inner Exception #%3$s) %4$s%5$s%6$s", text, System.lineSeparator(), i, _innerExceptions.get(i).toString(), "<---", System.lineSeparator());
        }
