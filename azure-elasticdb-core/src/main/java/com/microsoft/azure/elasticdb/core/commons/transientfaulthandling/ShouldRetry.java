 ...
 * @param delay         The delay that indicates how long the current thread will be suspended before the next iteration is invoked.
 * @return <see langword="true"/> if a retry is allowed; otherwise, <see langword="false"/>.
 */
@FunctionalInterface
public interface ShouldRetry {
    boolean invoke(int retryCount, RuntimeException lastException, ReferenceObjectHelper<Duration> delay);
}