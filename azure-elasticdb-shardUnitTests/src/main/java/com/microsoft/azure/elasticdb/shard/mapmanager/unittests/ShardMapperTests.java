...
public class ShardMapperTests {
  ...
  private static void cleanShardMapsHelper() {
    ...
    if (smm.tryGetListShardMap(ShardMapperTests.s_listShardMapName) != null) {
    ...
    if (smm.tryGetRangeShardMap(ShardMapperTests.s_rangeShardMapName) != null) {
  ...
  public static void ShardMapperTestsInitialize() throws SQLException {
    ...
    try {
      ...
      for (int i = 0; i < ShardMapperTests.s_shardedDBs.length; i++) {
        try (Statement stmt = conn.createStatement()) {
          String query =
              String.format(Globals.DROP_DATABASE_QUERY, ShardMapperTests.s_shardedDBs[i]);
          stmt.executeUpdate(query);
        } catch (SQLException ex) {
          ex.printStackTrace();
        }
        try (Statement stmt = conn.createStatement()) {
          String query =
              String.format(Globals.CREATE_DATABASE_QUERY, ShardMapperTests.s_shardedDBs[i]);
          stmt.executeUpdate(query);
        } catch (SQLException ex) {
          ex.printStackTrace();
        }
      ...
      ListShardMap<Integer> lsm =
          smm.createListShardMap(ShardMapperTests.s_listShardMapName, ShardKeyType.Int32);

      assert lsm != null;

      assert ShardMapperTests.s_listShardMapName == lsm.getName();

      // Create range shard map.
      RangeShardMap<Integer> rsm =
          smm.createRangeShardMap(ShardMapperTests.s_rangeShardMapName, ShardKeyType.Int32);

      assert rsm != null;

      assert ShardMapperTests.s_rangeShardMapName == rsm.getName();
    } catch (Exception e) {
  ...
  public void shardMapTypeFailures() {
    ...
    try {
      RangeShardMap<Integer> rsm =
          smm.<Integer>getRangeShardMap(ShardMapperTests.s_listShardMapName);
    ...
    try {
      ListShardMap<Integer> lsm =
          smm.<Integer>getListShardMap(ShardMapperTests.s_rangeShardMapName);
    ...
    try {
      ListShardMap<UUID> lsm = smm.<UUID>getListShardMap(ShardMapperTests.s_listShardMapName);
      fail("GetListShardMap did not throw as expected");
    } catch (ShardManagementException sme) {
      assert ShardManagementErrorCategory.ShardMapManager == sme.getErrorCategory();
      assert ShardManagementErrorCode.ShardMapTypeConversionError == sme.getErrorCode();
    ...
    try {
      RangeShardMap<Long> rsm = smm.<Long>getRangeShardMap(ShardMapperTests.s_rangeShardMapName);
      fail("GetRangeshardMap did not throw as expected");
    } catch (ShardManagementException sme) {
      assert ShardManagementErrorCategory.ShardMapManager == sme.getErrorCategory();
      assert ShardManagementErrorCode.ShardMapTypeConversionError == sme.getErrorCode();
    ...
    try {
      ListShardMap<UUID> lsm = smm.<UUID>getListShardMap(ShardMapperTests.s_rangeShardMapName);
      fail("GetListShardMap did not throw as expected");
    } catch (ShardManagementException sme) {
      assert ShardManagementErrorCategory.ShardMapManager == sme.getErrorCategory();
      assert ShardManagementErrorCode.ShardMapTypeConversionError == sme.getErrorCode();
  ...
  private <T> void addPointMappingDefault(List<T> keysToTest) throws SQLException {
    ...
    ShardLocation sl =
        new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapperTests.s_shardedDBs[0]);
  ...
  public void getPointMappingsForRange() {
    ...
    ListShardMap<Integer> lsm = smm.<Integer>getListShardMap(ShardMapperTests.s_listShardMapName);
    ...
    Shard s2 = lsm.createShard(
        new ShardLocation(Globals.TEST_CONN_SERVER_NAME, ShardMapperTests.s_shardedDBs[1]));
    assert s2 != null;
  ...
  public void addRangeMappingMultiple() {
    ...
    RangeShardMap<Integer> rsm =
        smm.<Integer>getRangeShardMap(ShardMapperTests.s_rangeShardMapName);
